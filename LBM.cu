...
*/
__host__ void initialization(double *r, double *c, double *fi, double *u, double *v, double *ex, double *ey, double *temp)
{
	// blocks in grid
										...
										double *u, double *v, double *ex, double *ey, double *temp)
{
	...
	temp0[gpu_field0_index(x, y)] = w0t*(omusq);
	...
	// Temperature
	cidot3u = tux;
	temp1[gpu_fieldn_index(x, y, 1)] = wst*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	cidot3u = tuy;
	temp1[gpu_fieldn_index(x, y, 2)] = wst*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	cidot3u = -tux;
	temp1[gpu_fieldn_index(x, y, 3)] = wst*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	cidot3u = -tuy;
	temp1[gpu_fieldn_index(x, y, 4)] = wst*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	...
	// Temperature
	cidot3u = tux + tuy;
	temp1[gpu_fieldn_index(x, y, 5)] = wdt*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	cidot3u = tuy - tux;
	temp1[gpu_fieldn_index(x, y, 6)] = wdt*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	cidot3u = -(tux + tuy);
	temp1[gpu_fieldn_index(x, y, 7)] = wdt*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	cidot3u = tux - tuy;
	temp1[gpu_fieldn_index(x, y, 8)] = wdt*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	...
	double *u, double *v, double *ex, double *ey, double *Temperature, double t,double *f0bc)
{
	...
	// Temperature
	cidot3u = tux + tuy;
	double tempe5 = wdT*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	cidot3u = tuy - tux;
	double tempe6 = wdT*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	cidot3u = -(tux + tuy);
	double tempe7 = wdT*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
	cidot3u = tux - tuy;
	double tempe8 = wdT*(omusq + cidot3u*(1.0 + 0.5*cidot3u));
...
__global__ void gpu_bc_charge(double *h0, double *h1, double *h2, double *temp0, double *temp1, double *temp2)
{
	...
	if (y == NY - 1) {
		temp0[gpu_field0_index(x, y)] = -temp0[gpu_field0_index(x, y)];
		temp1[gpu_fieldn_index(x, y, 3)] = -temp2[gpu_fieldn_index(x, y, 1)];
		temp1[gpu_fieldn_index(x, y, 4)] = -temp2[gpu_fieldn_index(x, y, 2)];
		temp1[gpu_fieldn_index(x, y, 1)] = -temp2[gpu_fieldn_index(x, y, 3)];
		temp1[gpu_fieldn_index(x, y, 2)] = -temp2[gpu_fieldn_index(x, y, 4)];
		temp1[gpu_fieldn_index(x, y, 7)] = -temp2[gpu_fieldn_index(x, y, 5)];
		temp1[gpu_fieldn_index(x, y, 8)] = -temp2[gpu_fieldn_index(x, y, 6)];
		temp1[gpu_fieldn_index(x, y, 5)] = -temp2[gpu_fieldn_index(x, y, 7)];
		temp1[gpu_fieldn_index(x, y, 6)] = -temp2[gpu_fieldn_index(x, y, 8)];
	}
}
...
void save_data_tecplot(FILE *fout, double time, double *rho_gpu, double *charge_gpu, double *phi_gpu,
	...
	if (first)
	{
		char str[] = "VARIABLES=\"x\",\"y\",\"u\",\"v\",\"p\",\"charge\",\"phi\",\"Ex\",\"Ey\",\"vorticity\",\"curlForce\",\"vorticity_diff\",\"XcurlForce\",\"Temperature\"";
		fprintf(fout, "%s\n", str);
	}
	fprintf(fout, "\n");
